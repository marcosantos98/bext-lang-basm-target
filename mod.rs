...
pub unsafe fn new(a: *mut arena::Arena, args: *const [*const c_char]) -> Option<*mut c_void> {
    ...
    let params = &[Param {
        name: c!("libpath"),
        description: c!("Path to implementation of extrn functions"),
    ...
    if let Err(message) = parse_args(params, args) {
        log(Log_Level::ERROR, c!("Parse arg error: %s"), message);
        return None;
    }

    if strcmp((*gen).libpath, c!("")) == 0 {
        log(Log_Level::ERROR, c!("Provide path to dynamic library containing the extrns implementation. `-C libpath='path'"));
        return None;
    }
...
pub unsafe fn run_program(
    ...
    cmd_append! {
        cmd, c!("bme"), program_path, c!("-n"), (*gen).libpath,
    }
